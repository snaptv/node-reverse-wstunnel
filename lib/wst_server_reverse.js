    ...
    return function(request){
      ...
      if (uri.pathname == '/create') {
        ...
        request.wsConnectionForControll.on('close', function(reasonCode, description) {
          ...
          util.log("Close TCP server on port %s", address.port);
        ...
        request.tcpServer.on('error', (error) => {
          util.log("TCP Server error:", error.message);
        ...
        if (tcpConn) {
          ...
          util.log("Unknown connection ID: %s", headers.id);
        }
      }
